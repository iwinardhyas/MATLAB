...
hold on;
% Plot target X
plot(sim_data.Time, repmat(r(1), size(sim_data.Time)), '--b', 'DisplayName', 'Target X');

% Plot posisi Y
plot(sim_data.Time, sim_data.Output(:, 2), 'g', 'DisplayName', 'Y Position');
% Plot target Y
plot(sim_data.Time, repmat(r(2), size(sim_data.Time)), '--g', 'DisplayName', 'Target Y');

% Plot posisi Z
plot(sim_data.Time, sim_data.Output(:, 3), 'r', 'DisplayName', 'Z Position');
% Plot target Z
plot(sim_data.Time, repmat(r(3), size(sim_data.Time)), '--r', 'DisplayName', 'Target Z');

% Tambahkan legenda, label, dan judul
legend show;
xlabel('Time (s)');
ylabel('Position');
title('Positions X, Y, Z vs Targets');
grid on;
hold off;
