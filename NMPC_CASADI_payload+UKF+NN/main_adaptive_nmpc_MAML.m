...
for k = 0:N-1
    ...
    R = diag([0.03, 0.05, 0.1, 0.1]); % Bobot upaya kontrol
...
%% === Inisialisasi MAML ===
net = initialize_maml_nn();      % Network MAML
lr_inner = 0.005;                % Learning rate inner loop MAML
lr_outer = 0.001;                % Learning rate outer loop MAML
history_residual_error = zeros(1, N_sim);
u_prev = zeros(nu, 1);           % Input awal untuk MAML
...
% Assign dynamics_fcn ke model quadrotor berbasis MATLAB
dynamics_fcn = @(x, u, g_val, l_val, Ixx_val, Iyy_val, Izz_val) ...
    quadrotor_dynamics_for_ukf(x, u, g_val, l_val, Ixx_val, Iyy_val, Izz_val, current_sim_mass);
...
for i = 1:N_sim
    ...
    mass_est = current_sim_mass;
    % === Prediksi residual dari MAML ===
    x_input_maml = [current_state; u_prev; mass_est]'; % 1 x (nx+nu)
    r_hat_dl = net.forward(dlarray(x_input_maml', 'CB'));
    r_hat = extractdata(r_hat_dl);
    
    % pisah residual
    state_residual = r_hat(1:12);
    mass_residual  = r_hat(13);
...
%     history_x(:, i+1) = current_state;
    ...
    % === Hitung residual aktual untuk update MAML ===
    % gunakan state_residual saat hitung next state aktual (ground truth simulasi)
    x_true_next = quadrotor_model_fisik_matlab(current_state, u_optimal, ...
                      current_sim_mass + mass_residual, dt, g, l, Ixx, Iyy, Izz, state_residual);
    x_fisik_pred = quadrotor_model_fisik_matlab(current_state, u_optimal, current_sim_mass, ...
                     dt, dynamics_fcn, g, l, Ixx, Iyy, Izz, []);
                 
    state_residual = x_true_next - x_fisik_pred;
    mass_residual_actual  = (current_sim_mass + 0) - current_sim_mass; % biasanya 0 unless mass changed externally
    y_true_maml = [state_residual; mass_residual]';
    
    [net, current_error] = update_maml_online(net, x_input_maml, y_true_maml, lr_inner, lr_outer);
    history_residual_error(i) = current_error;
    
    current_state = x_true_next;
    u_prev = u_optimal;
...
% PlotTrajectory;
figure;
plot(1:N_sim, history_residual_error, 'LineWidth', 1.5);
xlabel('Time Step'); ylabel('Residual Prediction Error (MSE)');
title('Online MAML Residual Learning Error');
grid on;
...
%% --- wrapper: RK4 + optional residual state correction ---
function x_next = quadrotor_model_fisik_matlab(x_current, u_applied, mass_val, dt_val, g_val, l_val, Ixx_val, Iyy_val, Izz_val, state_residual)
    % Buat dynamics handle (masuk ke rk4)
    dynamics_fcn = @(x,u,g,l,Ixx,Iyy,Izz) quadrotor_dynamics_for_maml(x, u, g, l, Ixx, Iyy, Izz, mass_val);

    % Integrasi RK4 (pastikan rk4_step_quad ada di path)
    x_pred = rk4_step_quad(x_current, u_applied, dt_val, dynamics_fcn, g_val, l_val, Ixx_val, Iyy_val, Izz_val);

    % Tambahkan residual state (panjang 12) jika ada
    if nargin >= 10 && ~isempty(state_residual)
        % state_residual harus 12x1 (atau 12xN)
        x_next = x_pred + reshape(state_residual, size(x_pred));
    else
        x_next = x_pred;
    end
end
...
%% --- fungsi dinamika continuous (untuk RK4) ---
function xdot = quadrotor_dynamics_for_maml(x, u, g, l, Ixx, Iyy, Izz, mass)
    % x: 12x1 [px;py;pz;phi;theta;psi;vx;vy;vz;p;q;r]
    % u: 4x1 motor thrust (N) atau input sejenis (sesuaikan koef)
    % mass: scalar massa (kg)
    %
    % Output:
    %   xdot: 12x1

    % --- mapping input ke gaya/torque (sesuaikan koef jika perlu) ---
    F_total = sum(u);                         % total thrust (N)
    tau_phi   = l * (u(2) - u(4));            % roll torque
    tau_theta = l * (u(3) - u(1));            % pitch torque
    tau_psi   = 0.05 * (u(1) - u(2) + u(3) - u(4)); % yaw torque coeff (sesuaikan)
    ...
    % --- rotasi body -> inertial (sama seperti yang kamu punya) ---
    R_b_i = rotz(psi) * roty(theta) * rotx(phi);

    % --- percepatan linear di frame inersia ---
    accel_x = R_b_i(1,3) * F_total / mass;
    accel_y = R_b_i(2,3) * F_total / mass;
    accel_z = R_b_i(3,3) * F_total / mass - g;

    % --- percepatan angular (Euler) ---
    p_dot = (tau_phi + (Iyy - Izz) * q * r) / Ixx;
    q_dot = (tau_theta + (Izz - Ixx) * p * r) / Iyy;
    r_dot = (tau_psi + (Ixx - Iyy) * p * q) / Izz;
