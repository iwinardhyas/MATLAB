...
% Definisi obstacle (bisa banyak)
num_obs = 2;
obs_center = [2 4;   % x koordinat
              2 3;   % y koordinat
              0.5 1];% z koordinat
obs_radius = [0.5 0.7]; % radius tiap obstacle
% Bobot untuk obstacle avoidance
w_obs = 100;   % bisa kamu tuning (semakin besar → semakin kuat menghindar)
d_safe = 1.0;  % jarak aman minimum [m]


% Parameter sensor (contoh sensor menghadap ke depan sumbu x)
sensor.dir = [1;0;0];    
sensor.max_range = 5;     % meter
d_min = 0.4;              % jarak aman minimum

cost = SX(0);   % <-- penting: inisialisasi sebagai CasADi SX scalar
g_sensor = [];         % constraints list
...
% Main horizon loop
for k = 0:N-1
    % Di dalam horizon loop untuk cost NMPC
    pos = X(1:3,k); % posisi drone pada step k
    d_all = [];
    for j = 1:num_obs
        c = obs_center(:,j);
        r = obs_radius(j);
        d_all = [d_all, norm(pos - c) - r];
    end

    % Soft-min (supaya smooth)
    alpha = 10;
    d_soft = -1/alpha * log(sum(exp(-alpha * d_all)));

    % Tambahkan penalti jika lebih kecil dari d_min
    cost = cost + w_obs * (fmax(0, d_min - d_soft))^2;
...
%     pos_drone = current_state(1:3);
%     d_sensor = range_sensor_3D(pos_drone, obs_center, obs_radius, sensor);
% 
%     % Logging hasil sensor
%     history_sensor(i) = d_sensor;
