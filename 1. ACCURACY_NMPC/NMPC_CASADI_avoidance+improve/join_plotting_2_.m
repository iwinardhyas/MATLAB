...
%% --- Legend (informatif) ---
legend([h_actual_single, h_actual_multi, h_ref, h_obs, h_start_ref, h_finish_ref], 'Location','bestoutside');
...
legend([h_actual_single, h_actual_multi,h_ref, h_start_ref, h_finish_ref, h_obs], 'Location', 'northeast');
...
% --- Data Input Awal ---
N_actual_single = size(results_single.history_x, 2);
F_total_norm_log_single = zeros(1, N_actual_single); 
v_act_norm_single = zeros(1, N_actual_single); 

N_actual_multi = size(results_multi.history_x, 2);
F_total_norm_log_multi = zeros(1, N_actual_multi); 
v_act_norm_multi = zeros(1, N_actual_multi);

% --- KONSTANTA (Pastikan nilai ini identik dengan NMPC Anda) ---
k_att = 0.01;       % attractive gain
k_rep = 4.0;       % repulsive gain (tune)
d0    = 4.0;       % influence distance of obstacles [m]
p_start = [0; 5]; %%% local minima2
p_goal = [33; 5];
% num_obs = size(obs_center, 2);

% LOOP POST-PROCESSING
for i = 1:N_actual_single
    % 1. Ambil Data Posisi & Hitung Kecepatan (NUMERIK)
    p_2D = results_single.history_x(1:2, i); 
    v_inertial = results_single.history_x(7:9, i);
    v_act_norm_single(i) = norm(v_inertial); 
    
    % 2. Hitung F_rep (NUMERIK)
    F_rep_total_2D = [0; 0];
    for j = 1:num_obs
        c = obs_center(1:2, j);
        r_obs = obs_radius(j);
        ...
        vec = p_2D - c;
        dist = norm(vec);      
        dist_surf = dist - r_obs; 
        
        % Kondisi pengaruh APF
        if dist_surf <= d0 && dist_surf > 1e-6 
            dir_ = vec / dist; 
            
            % Formula Gaya Repulsive Numerik (sesuai logika kode CasADi Anda)
            mag = k_rep * (1/dist_surf - 1/d0) * (1/dist_surf^2); 
            F_rep_total_2D = F_rep_total_2D + mag * dir_;
        end
    end
    
    % 3. Hitung F_att dan F_total (NUMERIK)
    % Formula Gaya Attractive Numerik
    F_att_2D = - k_att * (p_2D - p_goal);
    F_total_2D = F_att_2D + F_rep_total_2D;
    
    % 4. Log Besaran Gaya Total (NUMERIK)
    F_total_norm_log_single(i) = norm(F_total_2D);
end
...
for i = 1:N_actual_multi
    % 1. Ambil Data Posisi & Hitung Kecepatan (NUMERIK)
    p_2D = results_multi.history_x(1:2, i); 
    v_inertial_multi = results_multi.history_x(7:9, i);
    v_act_norm_multi(i) = norm(v_inertial_multi); 
    ...
    % 4. Log Besaran Gaya Total (NUMERIK)
    F_total_norm_log_multi(i) = norm(F_total_2D);
end

% --- Ambang Batas Deteksi Local Minima ---
E_F = 0.05;  % Ambang batas Gaya (misal, 0.05 Newton)
E_V = 0.1;   % Ambang batas Kecepatan (misal, 0.1 m/s)

% --- Deteksi Waktu Local Minima ---
% Logika: Kapan F_norm DAN v_norm di bawah ambang batas
is_trapped = (F_total_norm_log_single < E_F) & (v_act_norm_single < E_V);

% --- Plotting ---
figure(101); % Gunakan figure yang sama untuk perbandingan

% Subplot 1: Gaya APF Total
subplot(2,1,1);
hold on; grid on; box on;
plot(t_single, F_total_norm_log_single, 'Color', color1, 'LineWidth', 1.5, 'DisplayName', 'F adaptive R_{rate}');
plot(t_multi, F_total_norm_log_multi, 'Color', color2, 'LineWidth', 1.5, 'DisplayName', 'F fix R_{rate}');
yline(E_F, 'r--', 'LineWidth', 0.8, 'HandleVisibility', 'off'); 
ylabel('|F_{APF Total}| [N]');
title('Gaya APF Total dan Deteksi Local Minima');
legend('Location', 'northeast');

% Subplot 2: Kecepatan Drone
subplot(2,1,2);
hold on; grid on; box on;
plot(t_single, v_act_norm_single, 'Color', color1, 'LineWidth', 1.5, 'DisplayName', 'V adaptive R_{rate}');
plot(t_multi, v_act_norm_multi, 'Color', color2, 'LineWidth', 1.5, 'DisplayName', 'V fix R_{rate}');
yline(E_V, 'r--', 'LineWidth', 0.8, 'HandleVisibility', 'off'); 
ylabel('Speed [m/s]');
xlabel('Time [s]');
legend('Location', 'northeast');

% --- Penandaan (Highlight) Area Local Minima di Kedua Subplot ---

% Cek apakah ada kejadian local minima untuk di-highlight
if any(is_trapped)
    % Temukan awal dan akhir setiap kejadian
    start_indices = find(diff([0 is_trapped]) == 1);
    end_indices = find(diff([is_trapped 0]) == -1);

    % Highlight di kedua subplot
    for k = 1:length(start_indices)
        t_start = time(start_indices(k));
        t_end = time(end_indices(k));

        % Highlight area (Gaya)
        subplot(2,1,1);
        area(time, F_total_norm_log_single, 'LineStyle', 'none', 'FaceColor', [1 0.7 0.7], 'FaceAlpha', 0.3, 'XLim', [t_start, t_end]);
        area(time, F_total_norm_log_multi, 'LineStyle', 'none', 'FaceColor', [1 0.7 0.7], 'FaceAlpha', 0.3, 'XLim', [t_start, t_end]);

        % Highlight area (Kecepatan)
        subplot(2,1,2);
        area(time, v_act_norm_single, 'LineStyle', 'none', 'FaceColor', [1 0.7 0.7], 'FaceAlpha', 0.3, 'XLim', [t_start, t_end]);
        area(time, v_act_norm_multi, 'LineStyle', 'none', 'FaceColor', [1 0.7 0.7], 'FaceAlpha', 0.3, 'XLim', [t_start, t_end]);

        % Tampilkan pesan deteksi di command window
        disp(['[', label, '] Terjebak Local Minima: ', num2str(t_start, '%.2f'), 's - ', num2str(t_end, '%.2f'), 's']);
    end
end
