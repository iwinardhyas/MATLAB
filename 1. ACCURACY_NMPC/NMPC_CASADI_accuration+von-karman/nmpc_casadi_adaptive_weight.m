...
wind_data = [ug, vg, wg];
% wind_data = repmat([2.0, 0, 0], size(wind_data, 1), 1); % 5 m/s arah X saja
...
% base_wy = 100;
% base_wz = 100;
base_wx = 1e5; % Sesuaikan dengan Q(1,1) Anda
base_wy = 1e5; % Sesuaikan dengan Q(2,2) Anda
base_wz = 1e4; % Sesuaikan dengan Q(3,3) Anda
...
n_wind = 3;
n_params = nx + nx*(N+1)+n_wind*N;  % [x0; xref_0; xref_1; ... xref_N]
...
wind_start_idx = nx + nx*(N+1) + 1;
wind_end_idx   = nx + nx*(N+1) + n_wind*N;
wind_param = reshape(all_params_sym(wind_start_idx:wind_end_idx), n_wind, N);
...
wind_nominal = [0; 0; 0]; % Sesuai dengan test case Anda

% 2. Loop sepanjang horizon
for k = 1:N
    % Control guess (hover)    
    u_guess = thrust_hover_value * ones(nu,1);
    arg_w0 = [arg_w0; u_guess];
    
    % Intermediate states (multiple shooting)
    for m = 1:M-1
        x_guess = full(F_sub(x_guess, u_guess, wind_nominal));
        arg_w0 = [arg_w0; x_guess];
    end
...
for k = 0:N-1
    ...
    w = {w{:}, U_vars{k+1}};
    lbw = [lbw; 0.05*thrust_hover_value*ones(nu,1)]; % Min 10% hover thrust
    ubw = [ubw; 2.5*thrust_hover_value*ones(nu,1)];   % Max 300% hover thrust
    ...
    for m = 1:M-1
        ...
        X_predicted = F_sub(X_current, U_vars{k+1},wind_param(:, k+1));
    ...
    X_predicted_final = F_sub(X_current, U_vars{k+1},wind_param(:, k+1));
    g = {g{:}, X_predicted_final - X_vars{k+2}};
    lbg = [lbg; zeros(nx,1)];
    ubg = [ubg; zeros(nx,1)];
    
    % Cost function - CONSERVATIVE tuning untuk stabilitas
    Q = diag([1e6, 1e6, 1e5, ... % px, py, pz
                   200, 200, 50, ...   % phi, theta, psi
                   10, 10, 10, ...      % vx, vy, vz
                   5, 5, 5]);  % p, q, r
    R = diag([0.01, 0.01, 0.01, 0.01]); % Bobot upaya kontrol
...
% Terminal cost
if improvement_tc == false
    Qf = 10 * Q; % Higher terminal weight
else
    k_adapt = 0.5; 
    adapt_factor = 1 + k_adapt * tracking_error;
    % (opsional: batasi agar tidak terlalu besar)
    adapt_factor = min(adapt_factor, 3);  % misal maksimum 3x lipat
...
for i = 1:N_sim
    ...
    wind_horizon_data = wind_data(i : i+N-1, :); 
    
    % Build parameter vector
    rate_penalty_value = 0.1;
    X_ref_horizon = generate_reference_horizon(current_time, N, dt, @QuadrotorReferenceTrajectory4);
    actual_params = [current_state; reshape(X_ref_horizon, [], 1); reshape(wind_horizon_data, [], 1)];
...
%     end
    ...
    current_state = full(F_discrete(current_state, u_optimal,wind_data(i,:)));
...
if save_data == true
    if improvement_tc == true
        ...
        results.dt = dt;
        results.method = 'adaptive';
        save('adaptive.mat','results');
    else
        results.history_x = history_x;
        results.history_u = history_u;
        results.history_x_ref = history_x_ref;
