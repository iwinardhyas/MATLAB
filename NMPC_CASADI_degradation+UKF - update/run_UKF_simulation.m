% function [estimated_mass, actual_mass] = run_UKF_simulation(ukf_Q, ukf_R)
%     % Parameter tetap
%     nominal_mass_value = 1.0;
%     added_payload_mass = 0.5;
%     time_of_payload_change = 5.0;
%     actual_mass_at_start = nominal_mass_value;
%     actual_changed_mass = actual_mass_at_start + added_payload_mass;
%     dt_ukf = 0.05;
%     T_sim = 10;
%     N_sim_total = T_sim / dt_ukf;
%     true_grav = 9.81;
% 
%     % State: [z; vz; m]
%     x_est = [0; 0; nominal_mass_value];
%     P = diag([0.1, 0.1, 1.0]);
% 
%     estimated_mass = zeros(1, N_sim_total+1);
%     actual_mass = zeros(1, N_sim_total+1);
% 
%     current_z = 0; current_vz = 0; current_mass = actual_mass_at_start;
%     estimated_mass(1) = x_est(3);
%     actual_mass(1) = current_mass;
% 
%     % UKF Dynamics dan Measurement
%     ukf_f = @(x,u,g)[x(2); (u - x(3)*g)/x(3); 0];
%     ukf_h = @(x)x(1);  % hanya mengukur z
%     alpha = 0.5; kappa = 0; beta = 2;
% 
%     for i = 1:N_sim_total
%         t = (i-1)*dt_ukf;
% 
%         % Simulasi perubahan payload
%         if t >= time_of_payload_change && current_mass == actual_mass_at_start
%             current_mass = actual_changed_mass;
%         end
%         actual_mass(i+1) = current_mass;
% 
%         % Thrust konstan
%         u_applied = 14; % gaya dorong konstan
%         z_noise = randn * sqrt(ukf_R(1,1));
%         z_measured = current_z + z_noise;
% 
%         % UKF prediction
%         [x_pred, P_pred] = ukf_predict(x_est, P, u_applied, ukf_f, ukf_Q, dt_ukf, true_grav, alpha, kappa, beta);
%         [x_est, P] = ukf_update(x_pred, P_pred, z_measured, ukf_h, ukf_R, alpha, kappa, beta);
%         estimated_mass(i+1) = x_est(3);
% 
%         % Simulasi gerak aktual drone
%         acc = (u_applied - current_mass*true_grav)/current_mass;
%         current_vz = current_vz + acc * dt_ukf;
%         current_z = current_z + current_vz * dt_ukf;
%     end
% end

function [estimated_mass_for_nmpc, history_m_actual] = run_UKF_simulation(ukf_Q, ukf_R)
    ...
    actual_mass_at_start = nominal_mass_value;
    added_payload_mass = 0.5; % kg (payload tambahan)
    ...
    ukf_x_est = [0.1; 0.0; nominal_mass_value + 0]; % Tebakan awal z, vz, dan massa (sedikit salah)
