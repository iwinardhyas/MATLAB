...
nx_ukf = nx_drone + 2; % [z; vz; mass] -> State yang diestimasi UKF
...
ukf_x_est = [0.1; 0.0; nominal_mass_value + 0.3 ; 1.0]; % Tebakan awal z, vz, dan massa (sedikit salah)

% Kovariansi State Awal P (ukuran nx_ukf x nx_ukf)
ukf_P = diag([0.2, 0.2, 0.5, 0.05]); % Ketidakpastian awal z, vz, dan massa (massa lebih tidak yakin)
...
ukf_Q = diag([1e-6, 1e-7, 1e-4, 1e-5]);  % Tambahkan noise kecil untuk η₁ (misal random walk lambat)
...
history_eta1_ukf_est = zeros(1, N_sim_total + 1);
history_eta1_ukf_est(1) = ukf_x_est(4);
...
for i = 1:N_sim_total
    ...
    true_eta1 = 1.0;
    if current_time >= 5.0
        true_eta1 = 0.6;  % contoh kerusakan propeller ke-1
    end
    ...
    history_eta1_ukf_est(i+1) = ukf_x_est(4);
    ...
    actual_x_next = rk4_step([current_actual_z; current_actual_vz], true_eta1 *u_optimal_nmpc, ...
...
subplot(4,1,4);
plot(time_vec, history_eta1_ukf_est, 'g--', 'LineWidth', 1.5);
title('Estimasi Efisiensi Aktuator η₁');
xlabel('Waktu (s)');
ylabel('η₁ (0–1)');
grid on;