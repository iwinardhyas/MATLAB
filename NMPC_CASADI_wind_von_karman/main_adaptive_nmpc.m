...
% Percepatan linear
accel_x = R_b_i(1,3) * F_total / MASS_f_sym + wind_input(1);
accel_y = R_b_i(2,3) * F_total / MASS_f_sym + wind_input(2);
accel_z = R_b_i(3,3) * F_total / MASS_f_sym - g + wind_input(3);
...
f = Function('f', {x, u, MASS_f_sym, wind_input}, {xdot}, ...
             {'x','u','mass','wind'}, {'xdot'});
...
k1 = f(X, U, MASS_F_discrete_sym, WIND_sym);
k2 = f(X + dt/2*k1, U, MASS_F_discrete_sym, WIND_sym);
k3 = f(X + dt/2*k2, U, MASS_F_discrete_sym, WIND_sym);
k4 = f(X + dt*k3, U, MASS_F_discrete_sym, WIND_sym);
X_next = X + dt/6*(k1 + 2*k2 + 2*k3 + k4);
F_discrete = Function('F_discrete', {X, U, MASS_F_discrete_sym, WIND_sym}, {X_next});
...
X_ref_param_local = all_params_sym(nx+1 : 2*nx);
nominal_mass_param = all_params_sym(2*nx+1);

% wind_start = 2*nx + 2;
% wind_end = wind_start + N * n_wind - 1;
% wind_param_flat = all_params_sym(wind_start : wind_end);
% wind_param_matrix = reshape(wind_param_flat, n_wind, N)'; % (N x 3)


wind_param = reshape(all_params_sym(nx+nx+2:end), 3, N); % Ukuran 3 x N
...
for k = 0:N-1
    ...
    g = {g{:}, F_discrete(X_vars{k+1}, U_vars{k+1}, nominal_mass_param, wind_param(:,k+1)) - X_vars{k+2}};
...
for i = 1:N_sim
    ...
    history_x_ref(:, i) = x_ref_at_current_time; % Simpan di kolom 'i'
        ...
        % Ambil wind_horizon aman (ukuran 3 x N)
    if i + N - 1 <= size(wind_data, 1)
        wind_horizon = wind_data(i : i+N-1, :)';
    else
        % Back-up aman kalau wind_data terlalu pendek
        remaining = size(wind_data, 1) - i + 1;
        tail_data = wind_data(i:end, :)';
        pad_data = repmat(wind_data(end,:)', 1, N - remaining);
        wind_horizon = [tail_data, pad_data];
    end
    ...
    actual_params = [current_state; x_ref_at_current_time; nominal_mass_value;wind_horizon(:)]; % Gunakan referensi yang diperbarui
    % Panggil solver
...
% Perbaikan plot wind:
t_vec = 0:dt:T_sim-dt;
figure('Name', 'Gangguan Angin Von Karman');
plot(t_vec, history_wind(1,:), 'r', t_vec, history_wind(2,:), 'g', t_vec, history_wind(3,:), 'b');
xlabel('Time (s)'); ylabel('Wind Disturbance (m/s)');
legend('u_w', 'v_w', 'w_w');
title('Von K\"arman Wind Disturbance');
grid on;
