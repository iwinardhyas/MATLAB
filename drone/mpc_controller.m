function mpc_ctrl = mpc_controller(sys, Ts)
    ...
    y_min = [-5; -5; -5; -5; -5; -5]; % Batasan output minimum
    y_max = [5; 5; 5; 5; 5; 5];     % Batasan output maksimum
    
    % Membuat objek MPC
     mpc_ctrl = mpc(sys, Ts, prediction_horizon, control_horizon);
    
    % Horizon MPC
    mpc_ctrl.PredictionHorizon = 20;
...
%     mpc_ctrl.MV.Max = u_max;
    ...
    % Bobot
    mpc_ctrl.Weights.ManipulatedVariables = 0.01;      % Bobot untuk MV
    mpc_ctrl.Weights.ManipulatedVariablesRate = 0.1;   % Bobot untuk perubahan MV
    mpc_ctrl.Weights.OutputVariables = [1, 1, 1, 1];      % Bobot untuk setiap output
end
