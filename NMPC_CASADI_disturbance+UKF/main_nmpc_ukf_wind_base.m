...
nx_ukf = 15; % [px, py, pz, phi, theta, psi, vx, vy, vz, p, q, r]
...
ny_ukf = 9;  % Hanya posisi & orientasi: [px; py; pz; phi; theta; psi]
ukf_R = diag([0.5, 0.5, 0.5, ... % px, py, pz
              0.1, 0.1, 0.1,... % phi, theta, psi
              0.2, 0.2, 0.2]); % vx, vy, vz
...
history_wind_actual = zeros(3, N_sim_total); % Menyimpan angin aktual
history_wind_est = zeros(3, N_sim_total); % Menyimpan angin aktual
...
accel_x = R_b_i(1,3) * F_total / nominal_mass_value + wind_param(1);
accel_y = R_b_i(2,3) * F_total / nominal_mass_value + wind_param(2);
accel_z = R_b_i(3,3) * F_total / nominal_mass_value - true_grav + wind_param(3);
...
f_quadrotor = Function('f_quadrotor',{x_sym,u_sym,wind_param},{xdot},{'x','u','wind'},{'xdot'});

% Diskretisasi Model (Menggunakan metode Runge-Kutta ke-4)
X_rk4 = MX.sym('X_rk4', nx, 1);
U_rk4 = MX.sym('U_rk4', nu, 1);
Wind_rk4 = MX.sym('Wind_rk4', 3);
XDOT_rk4 = f_quadrotor(X_rk4, U_rk4, Wind_rk4);
k1 = XDOT_rk4;
k2 = f_quadrotor(X_rk4+ dt_nmpc/2*k1, U_rk4, Wind_rk4);
k3 = f_quadrotor(X_rk4+ dt_nmpc/2*k2, U_rk4, Wind_rk4);
k4 = f_quadrotor(X_rk4+ dt_nmpc*k3, U_rk4, Wind_rk4);
X_next_rk4 = X_rk4+ dt_nmpc/6*(k1 + 2*k2 + 2*k3 + k4);
F_discrete_quadrotor = Function('F_discrete_quadrotor', {X_rk4, U_rk4, Wind_rk4}, {X_next_rk4});
...
Estimated_Wind_Param = all_params_nmpc_sym(2*nx+1 : 2*nx+3);
...
for k = 0:N_nmpc-1
    ...
    w = {w{:}, U_vars{k+1}};
    lbw = [lbw; 0.2*ones(nu,1)];   % Batas bawah gaya dorong motor (min 0N)
    ubw = [ubw; 3.0*ones(nu,1)];  % Batas atas gaya dorong motor (misal, 20N)
    ...
    Q_cost = diag([200, 200, 500, ... % px, py, pz
    ...
    g = {g{:}, F_discrete_quadrotor(X_vars{k+1}, U_vars{k+1}, Estimated_Wind_Param) - X_vars{k+2}};
...
% Inisialisasi array penyimpanan estimasi angin
% wind_est = zeros(3, N_sim_total);
% wind_est_filtered = zeros(3, N_sim_total);
...
%     actual_wind = zeros(3,1);
    ...
    disp(['Length of ukf_x_est: ', num2str(length(ukf_x_est))]);
    
    disp("State UKF terdiri dari:")
    disp("Posisi      : " + num2str(norm(ukf_x_est(1:3))))
    disp("Kecepatan   : " + num2str(norm(ukf_x_est(4:6))))
    disp("Euler angle : " + num2str(norm(ukf_x_est(7:9))))
    disp("Omega       : " + num2str(norm(ukf_x_est(10:12))))
    disp("Wind        : " + num2str(norm(ukf_x_est(13:15))))
    ...
    % Ambil estimasi angin dari hasil UKF (misal x(13:15) = wind)
%     wind_hat_for_nmpc = ukf_x_est(13:15);
    wind_hat_for_nmpc = zeros(3,1); 
    history_wind_est(:, i) = wind_hat_for_nmpc;
%     wind_est(:,i) = wind_hat_for_nmpc;
% 
%     alpha = 0.1;
%     if i > 1
%         wind_est_filtered(:,i) = alpha * wind_hat_for_nmpc + (1 - alpha) * wind_est_filtered(:,i-1);
%     else
%         wind_est_filtered(:,i) = wind_hat_for_nmpc;
%     end
    disp(['Step ', num2str(i), ...
      ' | Wind: ', mat2str(actual_wind', 3), ...
      ' | Estimasi: ', mat2str(wind_hat_for_nmpc', 3)])
...
%         x0_for_nmpc = ukf_x_est(1:nx); % [px;py;pz;phi;theta;psi;vx;vy;vz;p;q;r]
        x0_for_nmpc = current_actual_x;
        
        % Referensi NMPC (Spiral Trajectory)
        x_ref_for_nmpc = QuadrotorReferenceTrajectory1(current_time);

        % Parameter untuk NMPC solver
        params_for_nmpc_solver = vertcat(x0_for_nmpc, x_ref_for_nmpc, wind_hat_for_nmpc);
        ...
        fprintf('Step %d: Solver time = %.4f s (dt = %.4f s)\n', i, solver_time, dt_nmpc);
        solver_times(i) = solver_time;
        
        u_optimal_nmpc = full(sol_nmpc.x(nx + 1 : nx + nu)); % Ambil U_0
    ...
    history_x_ref(:,i) = QuadrotorReferenceTrajectory1(current_time); % Log referensi
    ...
    actual_x_for_dynamics = [current_actual_x; actual_wind]; % Create 13-state vector
...
history_x_ref(:,N_sim_total+1) = QuadrotorReferenceTrajectory1(T_sim); % Log referensi terakhir
...
%% --- 1. Tracking Posisi (x, y, z) ---
figure;
subplot(3,1,1); plot(t, history_x_ref(1,:), 'r--', t, history_x_actual(1,:), 'b','LineWidth',1.5);
ylabel('x [m]'); legend('Reference','Actual'); grid on;
subplot(3,1,2); plot(t, history_x_ref(2,:), 'r--', t, history_x_actual(2,:), 'b','LineWidth',1.5);
ylabel('y [m]'); legend('Reference','Actual'); grid on;
subplot(3,1,3); plot(t, history_x_ref(3,:), 'r--', t, history_x_actual(3,:), 'b','LineWidth',1.5);
ylabel('z [m]'); xlabel('Time [s]'); legend('Reference','Actual'); grid on;
sgtitle('Posisi Quadrotor');

%% --- 2. Tracking Sudut Euler (phi, theta, psi) ---
figure;
subplot(3,1,1); plot(t, history_x_actual(4,:)*180/pi,'b','LineWidth',1.5); ylabel('\phi [deg]'); grid on;
subplot(3,1,2); plot(t, history_x_actual(5,:)*180/pi,'b','LineWidth',1.5); ylabel('\theta [deg]'); grid on;
subplot(3,1,3); plot(t, history_x_actual(6,:)*180/pi,'b','LineWidth',1.5); ylabel('\psi [deg]'); xlabel('Time [s]'); grid on;
sgtitle('Sudut Euler Quadrotor');

%% --- 3. Thrust (Input Kontrol) ---
figure;
for i = 1:4
    plot(t(1:size(history_u_nmpc,2)), history_u_nmpc(i,:), 'LineWidth',1.5); hold on;
end
xlabel('Time [s]'); ylabel('Thrust [N]');
legend('f1','f2','f3','f4'); grid on;
sgtitle('Input Thrust dari NMPC');

%% --- 4. Estimasi vs Angin Aktual ---
figure;
wind_label = {'Wind_x','Wind_y','Wind_z'};
for i = 1:3
    subplot(3,1,i);
    plot(t(1:size(history_wind_actual,2)), history_wind_actual(i,:), 'r--','LineWidth',1.5); hold on;
    plot(t(1:size(history_wind_est,2)), history_wind_est(i,:), 'b','LineWidth',1.5);
    ylabel(wind_label{i}); grid on;
    legend('True','Estimated');
end
xlabel('Time [s]');
sgtitle('Estimasi vs Gangguan Angin');

%% --- 5. Error Posisi ---
error_pos = history_x_ref(1:3,:) - history_x_actual(1:3,:);
figure;
plot(t, vecnorm(error_pos), 'k','LineWidth',1.5);
xlabel('Time [s]'); ylabel('||e_{pos}|| [m]');
title('Norma Error Posisi Total'); grid on;

%% --- 6. Error Angin ---
error_wind = history_wind_actual - history_wind_est;
figure;
for i = 1:3
    subplot(3,1,i);
    plot(t(1:size(error_wind,2)), error_wind(i,:), 'LineWidth',1.5);
    ylabel(['Error ', wind_label{i}]); grid on;
end
xlabel('Time [s]');
sgtitle('Error Estimasi Angin');

%% --- 7. Lintasan 3D (x, y, z) ---
figure;
plot3(history_x_ref(1,:), history_x_ref(2,:), history_x_ref(3,:), 'r--','LineWidth',1.5); hold on;
plot3(history_x_actual(1,:), history_x_actual(2,:), history_x_actual(3,:), 'b','LineWidth',1.5);
xlabel('X [m]'); ylabel('Y [m]'); zlabel('Z [m]');
legend('Reference','Actual'); grid on;
title('Lintasan 3D Quadrotor');
...
function xdot = quadrotor_dynamics_with_wind(x, u, g_val, l_val, Ixx_val, Iyy_val, Izz_val)
    % x : 15×1  = [pos(3); vel(3); euler(3); omega(3);  wind(3)]
    % u : 4×1   = [thrust total & torques]  (atau empat gaya motor, terserah model Anda)
    ...
    wind_x = x(13); wind_y = x(14); wind_z = x(15);   % angin yg diestimasi

    %% --- Parameter fisik tetap -----------------------------------------
    mass = 1.0;           % massa konstan (kg) – bisa ambil dari workspace
    ...
    tau_psi   = 0.05   * (u1 - u2 + u3 - u4);   % Kf∙d_prop

    %% --- Dinamika translasi ---------------------------------------------
    R_b_i = rotz(psi) * roty(theta) * rotx(phi);

    accel_x = R_b_i(1,3) * F_total / mass + wind_x;
    accel_y = R_b_i(2,3) * F_total / mass + wind_y;
    accel_z = R_b_i(3,3) * F_total / mass - g_val + wind_z;
    ...
    %% --- Model angin (random-walk / konstan) ----------------------------
    wind_dot = zeros(3,1);  % ganti bila ingin model proses angin ≠ 0

    %% --- Gabung turunan --------------------------------------------------
    xdot = [vx; vy; vz;                      % 1–3
            phi_dot; theta_dot; psi_dot;    % 4–6
            accel_x; accel_y; accel_z;      % 7–9
            p_dot; q_dot; r_dot;            % 10–12
            wind_dot];                      % 13–15
end
...
%     sqrtP = chol(P, 'lower');
    ...
    P = 0.5 * (P + P');  % Pastikan simetris

    try
        sqrtP = chol(P, 'lower');
    catch
        warning('Matriks P tidak positif definit, ditambahkan regularisasi.');
        P = P + 1e-6 * eye(size(P));
        sqrtP = chol(P, 'lower');
    end
    ...
    disp("Size of x in ukf_dynamics_fcn: " + num2str(size(X_sigma,1)))
    for i = 1:2*nx_ukf+1
...
% --- Reference Trajectory (Spiral) ---
% Define a spiral reference trajectory for the quadrotor
function x_ref = QuadrotorReferenceTrajectory(t, nominal_mass)
    % Target Hover position and orientation
    z_hover = 1.0; % Target hover altitude
    % Spiral parameters
    radius = 1.0;
    angular_speed = 0.5; % rad/s
    vertical_speed = 0.2; % m/s

    px_ref = radius * cos(angular_speed * t);
    py_ref = radius * sin(angular_speed * t);
    pz_ref = z_hover + vertical_speed * t; % Constant vertical speed

    % Orientation: yaw (psi) follows the path, others are zero for level flight
    psi_ref = angular_speed * t; % Orient towards the direction of travel
    phi_ref = 0;
    theta_ref = 0;

    % Linear velocities (derivatives of position)
    vx_ref = -radius * angular_speed * sin(angular_speed * t);
    vy_ref = radius * angular_speed * cos(angular_speed * t);
    vz_ref = vertical_speed;

    % Angular velocities (p, q, r)
    p_ref = 0; q_ref = 0; r_ref = angular_speed; % Yaw rate
    
    x_ref = [px_ref; py_ref; pz_ref; phi_ref; theta_ref; psi_ref; ...
             vx_ref; vy_ref; vz_ref; p_ref; q_ref; r_ref];
end
