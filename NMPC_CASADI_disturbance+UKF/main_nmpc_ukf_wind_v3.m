...
nx = 15; % [x,y,z,phi,theta,psi,vx_inertial,vy_inertial,vz_inertial,p,q,r,Wx,Wy,Wz]
nu = 4;  % [f1, f2, f3, f4]
...
x0 = zeros(nx,1);
P0 = blkdiag(eye(12)*0.1, eye(3)*0.5);
% Q0 = 0.01*eye(nx);
% R0 = 0.05*eye(nz);
...
ukf_x_est(13:15) = [0;0;0]; % asumsi awal angin nol
P0 = blkdiag(eye(12)*0.1, eye(3)*0.5); % var awal wind lebih besar
...
az_inertial = thrust_inertial(3) / m - true_grav;
...
n_params = nx + nx*(N+1)+3*N;  % [x0; xref_0; xref_1; ... xref_N]
...
%     actual_wind = zeros(3,1);
    ...
    [x_store, ukf_Q, ukf_R, innovation_hist, ukf_P] = runRAUKF(...
    x0, P0, Q0, R0, measurement_y, dt, chi2_thr, lambda0, delta0, a, b, ...
    @(x, dt) quadrotor_dynamics_with_wind(x, u_applied_prev, true_grav, l, Ixx, Iyy, Izz),ukf_measurement_fcn);
    ...
    x_ref_12 = QuadrotorReferenceTrajectory1(current_time);
    x_ref_at_current_time = [x_ref_12; 0; 0; 0];
    
    wind_est = ukf_x_est(13:15);
    W_val = repmat(wind_est, N, 1);  % misalnya estimasi konstan
%     W_val = zeros(3*N,1);
...
history_x_ref(:, N_sim + 1) = [QuadrotorReferenceTrajectory1(T_sim);0;0;0];
