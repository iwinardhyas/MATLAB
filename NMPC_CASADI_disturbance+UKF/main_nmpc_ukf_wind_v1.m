...
ny_ukf = 12;  % [px, py, pz, phi, theta, psi] — yang diukur
...
ukf_P = diag([...
    0.1, 0.1, 0.2, ...  % posisi
    0.5, 0.5, 1.0, ... % orientasi  
    0.1, 0.1, 0.1, ...  % kecepatan linier
    0.1, 0.1, 0.1]);  % kecepatan sudut
% Noise pada dinamika, dan random walk pada massa (sangat kecil untuk massa)
ukf_Q = diag([0.00005  0.00005  0.0005, ...    % posisi (naik 10x)
              0.001 0.001 0.005, ...  % orientasi
              0.0005   0.0005   0.0005,  ...   % kecepatan (naik 10x)
              0.001   0.001   0.001]);      % angular rate (tetap)

% Kovariansi Noise Pengukuran R (ukuran n_measurements x n_measurements)
% Mengukur posisi (px, py, pz) dan orientasi (phi, theta, psi)
ny = 9; % [x y z phi theta psi vx vy vz]
% Naikkan R untuk posisi dan orientasi
ukf_R = diag([1.0   1.0   1.0, ...    % posisi (px, py, pz) - naik 100x
              1.0   1.0   1.0, ...    % orientasi (phi, theta, psi) - naik 100x
              1.0   1.0   1.0, ...    % kecepatan linier (vx, vy, vz)
              0.001   0.001   0.001]);      % angular rate (p, q, r)
...
%     end
    ...
    measurement_y = ukf_measurement_fcn(current_state) + randn(ny_ukf, 1) .* sqrt(diag(ukf_R));
