...
% Kovariansi Proses Q (noise pada dinamika)
% Rekomendasi: Tingkatkan nilai secara signifikan, terutama untuk
% state yang mengalami divergensi (pz, vz, dan mass)
% Rekomendasi Q yang lebih moderat
% px, py, pz, phi, theta, psi, vx, vy, vz, p, q, r, mass
ukf_Q = diag([0.01, 0.01, 0.01, ...   % Posisi dan orientasi
              0.01, 0.01, 0.01, ...
              0.1, 0.1, 0.1, ...      % Kecepatan (umumnya lebih berisik)
              0.01, 0.01, 0.01, ...
              0.1]);                   % Massa (harus bisa menyesuaikan diri dengan perubahan)                  % mass (lebih kecil dan seimbang)

% Rekomendasi R yang lebih realistis
noise_std = [0.2; 0.2; 0.2; 0.05; 0.05; 0.05];
ukf_R = diag(noise_std.^2);
...
%     measurement_y = ukf_measurement_fcn(current_actual) + randn(ny_ukf, 1) .* sqrt(diag(ukf_R));
    measurement_noise = noise_std .* randn(ny_ukf, 1);
    measurement_y = ukf_measurement_fcn(current_actual) + measurement_noise;
