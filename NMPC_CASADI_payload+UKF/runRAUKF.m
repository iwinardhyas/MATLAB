...
function [x_est, P, Q, R, innovation] = runRAUKF( ...
    x_est, P, Q, R, measurement_y, dt, chi2_thr, lambda0, delta0, a, b, ...
    dynamics_fcn, measurement_fcn, varargin)
    


    % Dimensi
    nx = length(x_est);
    nz = length(measurement_y);
    ...
    X_pred = zeros(nx, size(Xsig, 2));
    for i = 1:size(Xsig, 2)
        X_pred(:, i) = dynamics_fcn(Xsig(:, i), dt, varargin{:});
    end
    x_pred = X_pred * W';
    P_pred = Q;
    for i = 1:size(Xsig, 2)
    ...
    %% 4. Update
    K = Pxz / Pzz;
    innovation = measurement_y - z_pred;
    x_est = x_pred + K * innovation;
    P = P_pred - K * Pzz * K';

    %% 5. Adaptive Q & R
    phi = innovation' * (Pzz \ innovation);
    if phi > chi2_thr
        lambda = max(lambda0, (phi - a * chi2_thr) / phi);
        Q = (1 - lambda) * Q + lambda * (K * innovation * innovation' * K');

        delta = max(delta0, (phi - b * chi2_thr) / phi);
        residual = measurement_y - measurement_fcn(x_est);
        Szz_hat = cov(Zsig');
        R = (1 - delta) * R + delta * (residual * residual' + Szz_hat);
        ...
        Pzz_corr = Szz_hat + R;
        K_hat = Pxz_corr / Pzz_corr;
        x_est = x_est + K_hat * (measurement_y - z_pred);
        P = Pxx_corr - K_hat * Pzz_corr * K_hat';
    end
end
